...
def list_ses_templates(page, size):
    try:
        ...
        logger.error(f"Failed to list SES templates for page {page} with size {size}: {e}")
        raise
...
def create_ses_template(template_data):
    try:
        ...
        logger.error(f"Failed to create SES template with name {template_data['TemplateName']}: {e}")
        raise
